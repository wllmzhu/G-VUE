...
class EnvBatch():
    ...
    def __init__(self, cfg, feature_store=None, batch_size=100):
        """
        ...
        if feature_store:
            if type(feature_store) is dict:     # A silly way to avoid multiple reading
                ...
                self.image_w = cfg.model.image_size
                self.image_h = cfg.model.image_size
                self.vfov = VFOV
    ...
    def getStates(self):
        ...
        :return: [ ((30, FEATURE_SIZE), sim_state) ] * batch_size
        """
        feature_states = []
...
class R2RBatch():
    ...
    def __init__(self, cfg, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 name=None):
        self.cfg = cfg
        ...
        if feature_store:
            ...
            self.feature_size = cfg.model.v_sizes[-1]
        self.data = []
        if tokenizer:
            self.tok = tokenizer
        scans = []
        for split in splits:
            for i_item, item in enumerate(load_datasets(cfg, [split])):
                if cfg.test_only and i_item == 64:
                    break
                if "/" in split:
                    try:
                        new_item = dict(item)
        ...
        self.angle_feature = utils.get_all_point_angle_feature(cfg)
        self.sim = utils.new_simulator(cfg)
    ...
    def _load_nav_graphs(self):
        ...
        self.graphs = load_nav_graphs(self.cfg, self.scans)
        self.paths = {}
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    angle_feat = utils.angle_feature(self.cfg, loc_heading, loc_elevation)
            ...
            for c in candidate:
                ...
                angle_feat = utils.angle_feature(self.cfg, c_new['heading'], c_new['elevation'])
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if feature is None:
                feature = np.zeros((DISCRETIZED_VIEWS, self.cfg.model.v_sizes[-1]))
