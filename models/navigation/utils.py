...
from models.navigation.constants import DISCRETIZED_VIEWS, TSV_FIELDNAMES, VFOV

# padding, unknown word, end of sentence
base_vocab = ['<PAD>', '<UNK>', '<EOS>']
padding_idx = base_vocab.index('<PAD>')

def load_nav_graphs(cfg, scans):
    ...
    graphs = {}
    for scan in scans:
        with open(os.path.join(cfg.simulator.data.conn_path, '%s_connectivity.json' % scan)) as f:
            G = nx.Graph()
            positions = {}
            data = json.load(f)
...
def load_datasets(cfg, splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open(os.path.join(cfg.train.data.r2r_dir, 'R2R_%s.json' % split)) as f:
...
# def build_vocab(cfg, splits=['train'], min_count=5, start_vocab=base_vocab):
#     ''' Build a vocab, starting with base vocab containing a few useful tokens. '''
#     count = Counter()
#     t = Tokenizer()
#     data = load_datasets(cfg, splits)
#     for item in data:
        ...
        with open(feature_store, "r") as tsv_in_file:     # Open the tsv file.
            ...
            for item in reader:
                ...
                features[long_id] = np.frombuffer(base64.decodestring(item['features'].encode('ascii')),
                                                   dtype=np.float32).reshape((DISCRETIZED_VIEWS, -1))   # Feature of long_id is (36, FEATURE_SIZE)
    else:
        features = None
...
def angle_feature(cfg, heading, elevation):
    ...
    return np.array([math.sin(heading), math.cos(heading),
                     math.sin(elevation), math.cos(elevation)] * (cfg.model.decoder.angle_feat_size // 4),
                    dtype=np.float32)

def new_simulator(cfg):
...
def get_point_angle_feature(cfg, baseViewId=0,):
    sim = new_simulator(cfg)
    sim.setNavGraphPath(cfg.simulator.data.conn_path)

    feature = np.empty((DISCRETIZED_VIEWS, cfg.model.decoder.angle_feat_size), np.float32)
    ...
    for ix in range(DISCRETIZED_VIEWS):
        ...
        feature[ix, :] = angle_feature(cfg, heading, state.elevation)
    return feature

def get_all_point_angle_feature(cfg):
    return [get_point_angle_feature(cfg, baseViewId) for baseViewId in range(36)]
...
#     drop_mask = drop_func(torch.ones(FEATURE_SIZE).cuda())
#     return drop_mask
...
def ndtw_initialize(cfg):
    ndtw_criterion = {}
    scan_gts_dir = os.path.join(cfg.train.data.r2r_dir, 'id_paths.json')
    ...
    for key in scan_gts:
        ...
        if path_scan_id not in all_scan_ids:
            ...
            ndtw_graph = ndtw_graphload(cfg, path_scan_id)
            ndtw_criterion[path_scan_id] = DTW(ndtw_graph)
    return ndtw_criterion

def ndtw_graphload(cfg, scan):
    ...
    A networkx graph.
    """
    connections_file = os.path.join(cfg.simulator.data.conn_path, '{}_connectivity.json'.format(scan))
