...
from models.r2r_decoder.constants import DISCRETIZED_VIEWS

from models.r2r_decoder.env import R2RBatch
import models.r2r_decoder.utils as utils
from models.r2r_decoder.utils import padding_idx, print_progress
import models.r2r_decoder.model_PREVALENT as model_PREVALENT
import models.r2r_decoder.model_OSCAR as model_OSCAR
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, cfg, env, results_path, tok, episode_len=20):
        ...
        # Optimizers
        if cfg.train.setting.optim == 'rms':
            print("Optimizer: Using RMSProp")
            cfg_optim = torch.optim.RMSprop
        elif cfg.train.setting.optim == 'adam':
            print("Optimizer: Using Adam")
            cfg_optim = torch.optim.Adam
        elif cfg.train.setting.optim == 'adamW':
            print("Optimizer: Using AdamW")
            cfg_optim = torch.optim.AdamW
        elif cfg.train.setting.optim == 'sgd':
            print("Optimizer: sgd")
            cfg_optim = torch.optim.SGD

        self.vln_bert_optimizer = cfg_optim(self.vln_bert.parameters(), lr=cfg.train.setting.lr)
        self.critic_optimizer = cfg_optim(self.critic.parameters(), lr=cfg.train.setting.lr)
        self.optimizers = (self.vln_bert_optimizer, self.critic_optimizer)

        # Evaluations
        self.losses = []
        self.criterion = nn.CrossEntropyLoss(ignore_index=cfg.model.decoder.crossentropy_ignoreid, size_average=False)
        self.ndtw_criterion = utils.ndtw_initialize(cfg)
    ...
    # def _feature_variable(self, cfg, obs):
    #     ''' Extract precomputed features into variable. '''
    #     features = np.empty((len(obs), DISCRETIZED_VIEWS, self.feature_size + cfg.model.decoder.angle_feat_size), dtype=np.float32)
    ...
    def _candidate_variable(self, obs):
        ...
        candidate_feat = np.zeros((len(obs), max(candidate_leng), self.feature_size + self.cfg.model.decoder.angle_feat_size), dtype=np.float32)
    ...
    def get_input_feat(self, obs):
        input_a_t = np.zeros((len(obs), self.cfg.model.decoder.angle_feat_size), np.float32)
        for i, ob in enumerate(obs):
            input_a_t[i] = utils.angle_feature(self.cfg, ob['heading'], ob['elevation'])
    ...
    def _teacher_action(self, obs, ended):
        ...
        for i, ob in enumerate(obs):
            if ended[i]:                                            # Just ignore this index
                a[i] = self.cfg.model.decoder.crossentropy_ignoreid
            else:
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.episode_len):
            ...
            if (t >= 1) or (self.cfg.model.decoder.type=='prevalent'):
            ...
            for i, next_id in enumerate(cpu_a_t):
                if next_id == (candidate_leng[i]-1) or next_id == self.cfg.model.decoder.crossentropy_ignoreid or ended[i]:    # The last action is <end>
        ...
        if train_rl:
            ...
            for t in range(length-1, -1, -1):
                discount_reward = discount_reward * self.cfg.train.a2c.gamma + rewards[t]  # If it ended, the reward will be 0
            ...
            if self.cfg.train.a2c.normalize_loss == 'total':
                rl_loss /= total
            elif self.cfg.train.a2c.normalize_loss == 'batch':
                rl_loss /= batch_size
            else:
                assert self.cfg.train.a2c.normalize_loss == 'none'
    ...
    def accumulate_gradient(self, feedback='teacher', **kwargs):
        if feedback == 'teacher':
            ...
            self.rollout(train_ml=self.cfg.train.setting.teacher_weight, train_rl=False, **kwargs)
        elif feedback == 'sample':
            self.feedback = 'teacher'
            self.rollout(train_ml=self.cfg.train.setting.ml_weight, train_rl=False, **kwargs)
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            if feedback == 'teacher':
                ...
                if self.cfg.train.setting.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(train_ml=self.cfg.train.setting.ml_weight, train_rl=False, **kwargs)
                self.feedback = 'sample'
                self.rollout(train_ml=None, train_rl=True, **kwargs)
            ...
            if self.cfg.train.data.aug_path is None:
