...
def generate_img_features(info):
    ...
    v_backbone_name = info['v_backbone_candidate']
    cfg = info['v_backbone_config'][v_backbone_name]
    v_backbone = build_v_backbone(cfg)

    is_ResNet = v_backbone_name.find('ResNet') != -1
    ...
    img_feature_tsv = os.path.join(info['img_feature_dir'], v_backbone_name + '_1.tsv')
...
def main(cfg):
    ...
    outfile_path = os.path.join(cfg.dataset.info.img_feature_dir, cfg.dataset.info['v_backbone_candidate'] + '_1.tsv')
