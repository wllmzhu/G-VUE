...
def camera_parameters(scan, cfg_var):
  ...
  intrinsics = {}
  extrinsics = {}
  with open(camera_template % (cfg_var['base_dir'],scan,scan)) as f:
    pos = -1
...
def depth_to_skybox(scan, cfg_var, visualize=VISUALIZE_OUTPUT, fill_holes=FILL_HOLES):

  # Load camera parameters
  intrinsics,extrinsics = camera_parameters(scan, cfg_var)
  ...
  for pano in pano_ids:
    ...
    for c in range(3):
      ...
      for i in range(6):
        name = '%d_%d' % (c,i)
        if visualize:
          rgb[name] = cv2.imread(color_template % (cfg_var['base_dir'],scan,pano,name))
        # Load 16bit grayscale image
        d_im = cv2.imread(depth_template % (cfg_var['base_dir'],scan,pano,name), cv2.IMREAD_ANYDEPTH)
    ...
    for skybox_ix in range(6):
      ...
      depth_small = cv2.resize(cv2.flip(base_depth, 1),(cfg_var['DOWNSIZED_WIDTH'],cfg_var['DOWNSIZED_HEIGHT']),interpolation=cv2.INTER_NEAREST) # flip around y-axis, downsize
      if fill_holes:
    ...
    if visualize:
      ...
      # Save output
      outfile = skybox_depth_template % (cfg_var['base_dir'],scan,pano)
...
#     p.starmap(depth_to_skybox, zip(scans, repeat(cfg_var)))
